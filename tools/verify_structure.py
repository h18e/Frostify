#!/usr/bin/env python3
"""Strukturpruefung fuer Frostify.
...
def check_swift_files() -> None:
    files = sorted(ROOT.glob("Frostify/**/*.swift")) + sorted(ROOT.glob("FrostifyTests/**/*.swift"))
    if not files:
...
def check_pbxproj() -> None:
    path = ROOT / "Frostify.xcodeproj" / "project.pbxproj"
    if not path.exists():
...
def check_referenced_build_files() -> None:
    text = (ROOT / "Frostify.xcodeproj" / "project.pbxproj").read_text(encoding="utf-8")
...
def load_model() -> dict[str, dict]:
    contents = list(ROOT.glob("Frostify/**/*.xcdatamodeld/*.xcdatamodel/contents"))
    if not contents:
...
def check_managed_properties(entities: dict[str, dict]) -> None:
    ...
    for path in sorted(ROOT.glob("Frostify/Model/Entities/*.swift")):
        text = path.read_text(encoding="utf-8")
        rel = path.relative_to(ROOT)
        ...
        if class_name not in ("Freezer", "FreezerSettings", "Item", "ConsumptionEvent", "CatalogProduct"):
            problem(f"{rel}: unerwartete Entitaetsklasse '{class_name}'.")
...
def check_keypaths(entities: dict[str, dict]) -> None:
    ...
    checked = 0
    for path in sorted(ROOT.glob("Frostify/**/*.swift")):
        text = path.read_text(encoding="utf-8")
...
def main() -> int:
    ...
    print("Frostify – Strukturpruefung\n")
    for line in CHECKS:
        print(f"  [ok] {line}")
    if PROBLEMS:
        print("\nGefundene Probleme:\n")
